...
def save_features(file_path: Union[Path, str]) -> Union[str, None]:  
...
if __name__ == "__main__":
    task = Task.init(project_name=settings.clearml.project,
                    task_name='Pipeline task feature engineer',
    ...
    with open(settings.params_path) as file:
        ...
        step_params = params.get('feature_engineer')
    if common_params:
        task.connect(common_params, name="common params")  
    if step_params:
        task.connect(step_params, name="feature engineer params")
    
    result = progress_map(save_features, input_filepath_files, 
