...
if __name__ == "__main__":
    task = Task.init(project_name=settings.clearml.project,
                    task_name='Pipeline task preprocess',
    ...
    with open(settings.params_path) as file:
        ...
        step_params = params.get('preprocessor')
    if common_params:
        task.connect(common_params, name="common params")  
    if step_params:
        task.connect(step_params, name="preprocessor params")
    
    input_filepath = get_abs_path(settings.storage.source_folder)
    file_type = r"/*csv"
