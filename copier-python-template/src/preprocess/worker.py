...
class PreprocessWorkerStep(BaseWorkerStep):
    def process(self, input_path: str, output_path: str) -> None:
        skip_mark: bool = bool(
            getattr(self, "_worker_params", {}).get("worker/skip_mark", False)
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            steps: list[tuple[str, Any]] = [("preprocessor", _build_preprocessor())]
            if not skip_mark:
...
def _build_preprocessor() -> "Preprocessor":  # type: ignore[name-defined]
    from src.preprocess.preprocessor import Preprocessor
    return Preprocessor()


def _build_mark_transformer() -> "MarkDataTransformer":  # type: ignore[name-defined]
